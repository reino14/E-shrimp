	...
	<div class="min-h-screen grid grid-cols-12">
		...
		<aside id="sidebar"
			...
			<nav class="space-y-1 text-sm">
				...
				<a href="{{ route('user.histori-data') }}" class="block px-3 py-2 rounded-lg bg-white border border-zinc-200 font-medium flex items-center gap-2">
					<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
					...
					<table class="w-full">
						...
						<tbody class="divide-y divide-zinc-200">
							...
							<tr>
								...
								<td class="px-4 py-3 text-sm">{{ $data->umur_budidaya ? 'Hari ke-' . $data->umur_budidaya : '-' }}</td>
							...
							@empty
							<tr>
								<td colspan="7" class="px-4 py-8 text-center text-sm text-zinc-500">Tidak ada data</td>
							</tr>
							@endforelse
						</tbody>
					</table>
				</div>
				<div class="px-4 py-3 border-t border-zinc-200 flex flex-col sm:flex-row items-center justify-between gap-4">
					<div class="flex items-center gap-2 text-sm text-zinc-600">
						<span>Tampilkan:</span>
						<select id="perPageSelect" class="px-3 py-1.5 rounded-lg border border-zinc-300 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-900 focus:border-transparent">
					...
					<div class="flex items-center gap-2">
						@if($historicalData->onFirstPage())
							<button disabled class="px-3 py-1.5 rounded-lg border border-zinc-300 text-sm text-zinc-400 cursor-not-allowed" disabled>
						...
						@else
							<a href="{{ $historicalData->previousPageUrl() }}" class="px-3 py-1.5 rounded-lg border border-zinc-300 text-sm hover:bg-zinc-50">
								<svg class="w-4 h-4 inline-block mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
									<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
								</svg>
								Prev
						...
						<span class="px-3 py-1.5 text-sm text-zinc-600">
							Halaman {{ $historicalData->currentPage() }} dari {{ $historicalData->lastPage() }}
						</span>
						
						@if($historicalData->hasMorePages())
							<a href="{{ $historicalData->nextPageUrl() }}" class="px-3 py-1.5 rounded-lg border border-zinc-300 text-sm hover:bg-zinc-50">
								Next
