	...
	<div class="min-h-screen grid grid-cols-12">
		...
		<aside id="sidebar"
			...
			<nav class="space-y-1 text-sm">
				...
				<a href="{{ route('user.artikel') }}" class="block px-3 py-2 rounded-lg bg-white border border-zinc-200 font-medium flex items-center gap-2">
					<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
		...
		<main class="col-span-12 md:col-span-10 p-4 sm:p-6 md:ml-0">
			<h1 class="text-lg sm:text-xl font-semibold mb-6">Daily Article</h1>
			<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
				@forelse($artikels as $artikel)
				<a href="{{ route('user.detail-artikel', $artikel->artikel_id) }}" class="block rounded-2xl border border-zinc-200 p-5 sm:p-6 hover:border-zinc-300 hover:shadow-md transition-all bg-white">
					<div class="aspect-video rounded-xl bg-zinc-100 mb-4 overflow-hidden">
						<img src="https://images.unsplash.com/photo-1559827260-dc66d52bef19?w=800&h=600&fit=crop" alt="{{ $artikel->judul }}" class="w-full h-full object-cover">
					</div>
					<h3 class="font-semibold mb-2 text-base sm:text-lg line-clamp-2">{{ $artikel->judul }}</h3>
					<p class="text-sm text-zinc-600 mb-3 line-clamp-3">{{ Str::limit($artikel->konten, 150) }}</p>
